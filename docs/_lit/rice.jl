...
chn2 = sample(constantrate(dag, bound, tree), NUTS(), 500)
...
# ancestral states?
M = mfun1(chn1[100], tree, bound)
A = DeadBird.AncestralSampler(M, bound)
mat, bound = ProfileMatrix(data, tree)
DeadBird.loglikelihood!(mat, M)
DeadBird.sample_ancestral(A, mat[1])

xs = DeadBird.sample_ancestral(y->mfun1(y, tree, bound), chn1, mat, 10)
